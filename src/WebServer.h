...
#ifndef HTTP_DOWNLOAD_UNIT_SIZE
...
public:
  ...
  void serveStatic(const char *uri, FS &fs, const char *path, const char *cache_header = NULL);
  ...
  size_t streamFile(T &file, const String &contentType, const int code = 200)
  {
    ...
    while (remaining > 0)
    {
      size_t read = min(remaining, 512);
      file.read(file_buffer, read);
      remaining -= read;
      _currentClient.writeFully(file_buffer, read);
    }
